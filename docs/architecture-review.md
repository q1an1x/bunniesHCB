...
新增房间级手动覆盖：开关/亮度操作暂停该房间相关策略，色温操作只暂停自适应色温，默认 30 分钟；反馈和软件回显不会产生覆盖。进入模式时保留已知的原自动化值，退出时只恢复仍由模式拥有的值；重复手动 OFF 也能阻止恢复。新模式和新的房间手动灯光输入不会被旧定时任务覆盖。模式动作可用 `--explain-mode` 离线预览，失败与启动恢复状态见 `health.json`。详细行为、边界和验收见 [家庭模式](house-modes.md)。
